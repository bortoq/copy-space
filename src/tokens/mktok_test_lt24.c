// mktok_test_lt24.c
// LT24 (unsigned A < B) test using 2a library word WORD_LT24 and fixed operand/result vars.
// Output: 6 bytes at TESTG[0..5]: 0x80 if (A<B) else 0x00
// Expected: 00 80 00 80 00 80
...
#define COPY1(dst_bitaddr, src_bitaddr) \
  ...
  struct pair { uint32_t A, B; } cases[6] = {
    { 0x000000u, 0x000000u },
    { 0x000000u, 0x000001u },
    { 0x000001u, 0x000000u },
    { 0x123456u, 0x123457u },
  ...
  for (unsigned i=0; i<6; i++) {
    ...
    // Store result byte: clear then set bit0 from VAR_LT
    uint64_t outb = (uint64_t)TESTG + (uint64_t)i*8u;
    CLRBYTE(outb);
    COPY1(outb + 0u, (uint64_t)VAR_LT);
  }
