...
bakdir="bak/cleanup_${ts}_fix_term0_desc_abi_dash"
mkdir -p "$bakdir"
...
if [ -z "$art_byte" ]; then
  echo "FAIL: cannot find ART(byte)=... in $log" >&2
  tail -n 200 "$log" >&2 || true
  exit 1
fi

# mkimage_meta normally produces one new tmp/*.bin; pick the first found after stamp
space_bin="$(find tmp -maxdepth 1 -type f -name '*.bin' -newer "$stamp" 2>/dev/null | head -n 1)"
if [ -z "$space_bin" ]; then
...
need(addr_bytes >= 1 and addr_bytes <= 8, f"unexpected addr_bytes={addr_bytes}")
