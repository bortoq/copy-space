...
bakdir="bak/cleanup_${ts}_fix_tdd_term0_desc_abi_out"
mkdir -p "$bakdir"
...
# mkimage_meta.sh requires --out
( sh scripts/mkimage_meta.sh --out tmp/tdd_term0 ) >"$log_meta" 2>&1 || {
...
if [ -z "$meta" ]; then
  echo "FAIL: no tmp/*.meta produced by mkimage_meta.sh --out tmp/tdd_term0" >&2
  echo "meta log:" >&2
  tail -n 200 "$log_meta" >&2 || true
  ls -la tmp >&2 || true
  exit 1
...
# Run meta runner (should run mkimage_std7_fixed and produce a .bin and logs with ART(byte)=...)
( sh "$meta" ) >"$log_run" 2>&1 || {
...
# ART(byte)=... should appear in some tmp/*.log (mkimage prints it to stderr)
