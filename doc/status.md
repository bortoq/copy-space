...
- [x] ABI artifacts doc sync: TDD `scripts/tdd/test_art_doc_sync.sh`
- [x] CI: `.github/workflows/ci.yml` (make bins / make test / make tdd / ABI doc sync)
...
- [x] Padding semantics: Forth0/TDD `src/forth0/tests/test_ptrprims_padding.f0`

### Derived pointer arithmetic (no new ART)
- [x] `INC_PTR32` (derived): `src/forth0/tests/test_incptr32.f0`
- [x] `ADD_PTR_CONST32` (derived): `src/forth0/tests/test_addptr_const32.f0`

### Derived pointer-based ops (via primitives)
- [x] `ADD24P` via prims: `src/forth0/tests/test_add24p_via_prims.f0`
- [x] `LT24P` via prims: `src/forth0/tests/test_lt24p_via_prims.f0`
- [x] `EQ24P` test: `src/forth0/tests/test_eq24p.f0`

### Alignment contract
- [x] Optional strict host-side check: `F0C_STRICT_ALIGN32=1` + TDD `scripts/tdd/test_forth0c_strict.sh`
...
- [x] Execution pipeline: mkimage → forth0c → vmrun(compile) → vmprep_forth0 → vmrun(run)
- [x] `make test` and `make tdd` run `.f0` tests (forth0-first)
- [x] Legacy C token-generators are optional: `make tok` / `make TOK=1 bins`
