...
bakdir="bak/cleanup_${ts}_add24p_test"
mkdir -p "$bakdir"
...
TEST_FILE="src/tokens/mktok_test_add24p_via_prims.c"
...
# Optional: backup if someone created an empty/partial file
# (we checked existence above)

cat > "$TEST_FILE" <<'EOF'
/* file: src/tokens/mktok_test_add24p_via_prims.c
 * date: 2026-05-05
 * purpose: todo B — ADD24 via block-pointer primitives (LOAD24AP/LOAD24BP/STORE24RP)
...
int main(int argc, char **argv) {
  ...
  /* todo B block-pointer primitives (ART 67..69) */
...
#define COPY1(dst_bitaddr, src_bitaddr) \
  ...
  for (unsigned k=0; k<4; k++) {
    uint64_t Aaddr  = OPBASE + (uint64_t)k*STRIDE*3u + 0u*STRIDE;
    uint64_t Baddr  = OPBASE + (uint64_t)k*STRIDE*3u + 1u*STRIDE;
    uint64_t RESaddr= OPBASE + (uint64_t)k*STRIDE*3u + 2u*STRIDE;

    /* write operands (24 bits) */
    SETN(1);
    ...
    EMIT(WORD_LITRP); write_be(f, (uint64_t)RESaddr, nb);

    /* compute via primitives */
    EMIT(WORD_LOAD24AP);
    EMIT(WORD_LOAD24BP);
    EMIT(WORD_ADD24);
    EMIT(WORD_STORE24RP);
    ...
    /* copy A,B,SUM,COUT to output */
    COPY24(base_out + 0u*8u, Aaddr);
    COPY24(base_out + 4u*8u, Baddr);
    COPY24(base_out + 8u*8u, VAR_SUM24);
    COPY1 (base_out + 12u*8u + 0u, VAR_COUT);
  }
