...
bakdir="bak/cleanup_${ts}_demo_db_improvements"
mkdir -p "$bakdir"
...
chmod +x "$F"
echo "OK: updated $F (backup: $bakdir/demo_db.sh.bak)" >&2
echo "Run (interactive): scripts/demo_db.sh" >&2
echo "Run (save file):   DEMO_CSV=tmp/demo.csv scripts/demo_db.sh > /dev/null" >&2
echo "Verbose benches:   VERBOSE=1 scripts/demo_db.sh" >&2
