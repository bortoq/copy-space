// mktok_test_eq24p.c
// EQ24P test: pointer-based equality using VAR_AP/VAR_BP + WORD_EQ24P.
// IMPORTANT: EQ24P assumes VAR_AP/VAR_BP are 32-bit aligned (bitaddr % 32 == 0).
//
...
int main(int argc, char **argv) {
  ...
  uint64_t WORD_EQ24P = vm_read_uint(&vm, ART + 62*(bitaddr_t)vm.addr_bits, vm.addr_bits);
...
#define COPY1(dst_bitaddr, src_bitaddr) \
  ...
  // Put operands in 4-byte aligned slots starting near TESTG.
  // We reserve:
  //  - bytes 0..3: outputs
  //  - bytes 4..15: unused
  //  - operands start from byte 16, but aligned up to 4 bytes boundary.
  uint64_t testg_byte = (uint64_t)(TESTG >> 3);
  uint64_t op_byte = (testg_byte + 16u + 3u) & ~3ull; // align to 4 bytes
  uint64_t OPBASE = op_byte << 3;

  uint64_t STRIDE = 8u*8u; // per case: A(4 bytes) + B(4 bytes)
  ...
  for (unsigned i=0; i<4; i++) {
    ...
    // Ensure padding bytes are defined
    ...
    // sanity (host-side): these must be 32-bit aligned
    // (we don't print it to keep logs stable)

    // set pointers (literal loaders)
    ...
    // store result bit into output byte bit0 (0x80)
    COPY1(TESTG + (uint64_t)i*8u + 0u, VAR_EQ);
  }
